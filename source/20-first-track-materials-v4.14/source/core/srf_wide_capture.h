...
typedef struct {
    ...
    uint16_t scroll_u, scroll_v;
    uint16_t textured, reserved;
} SrfWideCameraFace;
...
static void srf_observe_wide(void)
{
    ...
    if (key == 0x01918e && PIPE == 0xb5) {
        ...
        srf_wide_pending = 0;
        const int textured=(R3 & 0x8000)!=0;
        if (pointer < (textured?8u:2u) || count < 3 || count > 32 ||
            pointer+count*(textured?8u:4u)>65536u) return;
        ...
        if (GSU.pvRomBank && srf_wide_camera_count < 2048) {
            ...
            if(textured) {
                face->tex_bank=srf_wide_word(pointer-8)>>8;
                face->color=srf_wide_word(pointer-6)&255;
                face->color_mode=srf_wide_word(pointer-6)>>8;
                face->tex_base=srf_wide_word(pointer-4);
                face->tex_mask=srf_wide_word(pointer-2);
                face->scroll_u=srf_wide_word(0x00c0);
                face->scroll_v=srf_wide_word(0x00c2);
            }
            for (i = 0; i < count; ++i) {
                ...
                if(textured) {
                    face->uv[i*2]=srf_wide_word(pointer+i*8+4)/256.0f;
                    face->uv[i*2+1]=srf_wide_word(pointer+i*8+6)/256.0f;
                }
            }
