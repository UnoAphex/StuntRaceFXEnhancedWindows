...
#ifdef _WIN32
#define SRF_EXPORT extern "C" __declspec(dllexport)
#else
#define SRF_EXPORT extern "C" __attribute__((visibility("default")))
#endif

SRF_EXPORT const uint8_t *srf_get_gsu_ram(size_t *size)
{
    if (size)
        *size = SuperFX.pvRam ? (size_t) SuperFX.nRamBanks * 65536u : 0u;
    return SuperFX.pvRam;
}

SRF_EXPORT const uint8_t *srf_get_gsu_registers(size_t *size)
{
    if (size)
        *size = SuperFX.pvRegisters ? 0x300u : 0u;
    return SuperFX.pvRegisters;
}

SRF_EXPORT void srf_set_draw_distance(int mode)
{
    S9xSRFSetDrawDistance(mode);
}

SRF_EXPORT void srf_reset_gsu_trace(void)
{
    S9xSRFResetTrace();
}

void S9xSRFWideBackgroundEnable(bool enabled);
const uint8_t *S9xSRFWideBackground(uint32_t *size);
const uint8_t *S9xSRFWideWorldReference(uint32_t *size);
unsigned S9xSRFWideDisplaySlot();
void S9xSRFWideBackgroundReset();
void S9xSRFWideSpritesReset();
const uint8_t *S9xSRFWideSprites(uint32_t *size);
SRF_EXPORT const uint8_t *srf_get_wide_sprites(size_t *size) {
    uint32_t bytes=0; const uint8_t* data=S9xSRFWideSprites(&bytes);
    if(size) *size=bytes; return data;
}
const uint8_t *S9xSRFWideColors(uint32_t *size);
SRF_EXPORT void srf_reset_wide_capture(void) {
    S9xSRFResetWideCapture();
    S9xSRFWideBackgroundReset();
    S9xSRFWideSpritesReset();
    S9xSRFWideBackgroundEnable(false);
}
SRF_EXPORT const uint8_t *srf_get_wide_colors(size_t *size) {
    uint32_t bytes=0; const uint8_t* data=S9xSRFWideColors(&bytes);
    if(size) *size=bytes; return data;
}
SRF_EXPORT const uint8_t *srf_get_wide_display_camera(size_t *size) {
    uint32_t bytes=0;
    const uint8_t* data=S9xSRFGetWideReady(S9xSRFWideDisplaySlot(), &bytes);
    if(size) *size=bytes;
    return data;
}
SRF_EXPORT void srf_start_wide_capture(void) {
    S9xSRFStartWideCapture();
    // These observation addresses are verified only for the unmodified USA Rev 1.
    const bool supported = Memory.ROMCRC32 == 0x380c2635u;
    S9xSRFWideBackgroundEnable(supported);
    if (!supported) { uint32_t ignored; S9xSRFGetWideCapture(&ignored); }
}
SRF_EXPORT const uint8_t *srf_get_wide_background(size_t *size) {
    uint32_t bytes=0;
    const uint8_t* result=S9xSRFWideBackground(&bytes);
    if(size) *size=bytes;
    return result;
}
SRF_EXPORT const uint8_t *srf_get_wide_world_reference(size_t *size) {
    uint32_t bytes=0;
    const uint8_t* result=S9xSRFWideWorldReference(&bytes);
    if(size) *size=bytes;
    return result;
}
SRF_EXPORT const uint8_t *srf_get_wide_capture(size_t *size)
{
    uint32_t bytes = 0;
    const uint8_t *result = S9xSRFGetWideCapture(&bytes);
    // A subsequent frame must opt in again; native and normal play do no BG work.
    S9xSRFWideBackgroundEnable(false);
    if (size) *size = bytes;
    return result;
}
SRF_EXPORT const uint8_t *srf_get_cgram(size_t *size)
{
    if (size) *size = sizeof(PPU.CGDATA);
    return (const uint8_t *)PPU.CGDATA;
}
SRF_EXPORT const uint8_t *srf_get_wide_camera(size_t *size)
{
    uint32_t bytes = 0;
    const uint8_t *result = S9xSRFGetWideCamera(&bytes);
    if (size) *size = bytes;
    return result;
}
SRF_EXPORT const uint8_t *srf_get_wide_ppu(size_t *size)
{
    static uint16_t state[32];
    memset(state, 0, sizeof(state));
    state[0] = PPU.BGMode;
    state[1] = Memory.FillRAM[0x212c];
    for (int i = 0; i < 4; ++i) {
        state[2+i*6] = PPU.BG[i].SCBase;
        state[3+i*6] = PPU.BG[i].NameBase;
        state[4+i*6] = PPU.BG[i].SCSize;
        state[5+i*6] = PPU.BG[i].BGSize;
        state[6+i*6] = PPU.BG[i].HOffset;
        state[7+i*6] = PPU.BG[i].VOffset;
    }
    if(size) *size=sizeof(state);
    return (const uint8_t*)state;
}
SRF_EXPORT const uint8_t *srf_get_vram(size_t *size)
{
    if(size) *size=65536;
    return Memory.VRAM;
}
SRF_EXPORT const uint8_t *srf_get_wide_dma(size_t *size) {
    uint32_t bytes=0;
    const uint8_t* data=S9xSRFGetWideDMA(&bytes);
    if(size) *size=bytes;
    return data;
}

SRF_EXPORT void srf_start_geometry_probe(uint32_t watch)
{
    S9xSRFStartGeometryProbe(watch);
}

SRF_EXPORT const uint8_t *srf_get_geometry_probe(size_t *size, int snapshots)
{
    uint32_t bytes = 0;
    const uint8_t *result = S9xSRFGetGeometryProbe(&bytes, snapshots);
    if (size) *size = bytes;
    return result;
}

SRF_EXPORT void srf_reset_gsu_profile(void)
{
    S9xSRFResetProfileCounters();
}

SRF_EXPORT uint64_t srf_get_gsu_time_ns(void)
{
    return S9xSRFGetGSUTimeNS();
}

SRF_EXPORT uint64_t srf_get_gsu_instruction_count(void)
{
    return S9xSRFGetGSUInstructionCount();
}

SRF_EXPORT uint64_t srf_get_gsu_exec_calls(void)
{
    return S9xSRFGetGSUExecCalls();
}

SRF_EXPORT const uint8_t *srf_get_gsu_trace(size_t *size)
{
    uint32_t trace_size = 0;
    const uint8_t *trace = S9xSRFGetTrace(&trace_size);
    if (size)
        *size = trace_size;
    return trace;
}

SRF_EXPORT const uint8_t *srf_get_ram_write_trace(size_t *size)
{
    uint32_t trace_size = 0;
    const uint8_t *trace = S9xSRFGetRamWriteTrace(&trace_size);
    if (size)
        *size = trace_size;
    return trace;
}
