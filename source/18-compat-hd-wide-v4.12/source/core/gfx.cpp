...
#include "srf_wide_background.h"
#include "srf_wide_sprites.h"

void RenderLine (uint8 C)
{
	// The wide observers need the same per-scanline OBJ selection on rendered
	// and skipped frames. OAM normally changes during vblank, so building the
	// list here once also leaves the normal S9xUpdateScreen path with the same
	// prepared data it would have produced later in the frame.
	if (srf_wide_background_enabled && IPPU.OBJChanged)
		SetupOBJ();
	...
	srf_wide_background_line(C);
	srf_wide_sprite_line(C);
}
