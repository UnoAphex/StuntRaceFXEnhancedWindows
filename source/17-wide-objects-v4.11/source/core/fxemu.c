...
#define FX_STW(reg) \
	...
	S9xSRFTraceRamWrite(GSU.vLastRamAdr, (uint8_t) SREG, 0x800000u | (GSU.vPrgBankReg << 16) | R15); \
...
#define FX_SMS(reg) \
	...
	R15++; \
	FETCHPIPE; \
	S9xSRFTraceRamWrite(GSU.vLastRamAdr, (uint8_t) v, 0x800000u | (GSU.vPrgBankReg << 16) | R15); \
...
/* Frontend-facing draw-distance setting. Course-list mutation was deliberately
 * removed: those links also participate in game logic, so changing them here
 * can alter collisions and race timing. The frontend safely uses this setting
 * to control distant surface-detail coverage instead. */
static int srf_draw_distance_mode;
static uint8_t srf_trace[65536];
static uint32_t srf_trace_size;
static uint8_t srf_ram_write_trace[16384];
static uint32_t srf_ram_write_trace_size;
static int srf_trace_active;
static uint64_t srf_gsu_time_ns;
static uint64_t srf_gsu_instruction_count;
static uint64_t srf_gsu_exec_calls;

static uint64_t srf_profile_now_ns (void)
{
#if defined(_WIN32)
...
void S9xSRFSetDrawDistance (int mode)
{
	srf_draw_distance_mode = mode < 0 ? 0 : (mode > 2 ? 2 : mode);
}

void S9xSRFResetTrace (void)
{
	srf_trace_size = 0;
	srf_ram_write_trace_size = 0;
	srf_trace_active = 1;
}

void S9xSRFResetProfileCounters (void)
{
	srf_gsu_time_ns = 0;
	srf_gsu_instruction_count = 0;
	srf_gsu_exec_calls = 0;
}

uint64_t S9xSRFGetGSUTimeNS (void)
{
	return srf_gsu_time_ns;
}

uint64_t S9xSRFGetGSUInstructionCount (void)
{
	return srf_gsu_instruction_count;
}

uint64_t S9xSRFGetGSUExecCalls (void)
{
	return srf_gsu_exec_calls;
}

const uint8_t *S9xSRFGetTrace (uint32_t *size)
{
	if (size)
		*size = srf_trace_size;
	srf_trace_active = 0;
...
void S9xSRFTraceRamWrite (uint32_t address, uint8_t value, uint32_t pc)
{
	uint32_t record;
	uint16_t low = (uint16_t) address;
	unsigned i;
	static const uint16_t watched[] = {
		0x2128, 0x2152, 0x217c, 0x21a6, 0x21d0, 0x21fa, 0x2224,
		0x224e, 0x2278, 0x22a2, 0x22cc, 0x22f6, 0x2320
	};
	if (!srf_trace_active)
		return;
	for (i = 0; i < sizeof(watched) / sizeof(watched[0]); ++i)
		if (low == watched[i] || low == (uint16_t) (watched[i] + 1))
			break;
	if (i == sizeof(watched) / sizeof(watched[0]) ||
	    srf_ram_write_trace_size + 8 > sizeof(srf_ram_write_trace))
		return;
	record = srf_ram_write_trace_size;
	srf_ram_write_trace[record + 0] = (uint8_t) low;
	srf_ram_write_trace[record + 1] = (uint8_t) (low >> 8);
	srf_ram_write_trace[record + 2] = value;
	srf_ram_write_trace[record + 3] = 0;
	srf_ram_write_trace[record + 4] = (uint8_t) pc;
	srf_ram_write_trace[record + 5] = (uint8_t) (pc >> 8);
	srf_ram_write_trace[record + 6] = (uint8_t) (pc >> 16);
	srf_ram_write_trace[record + 7] = 0;
	srf_ram_write_trace_size += 8;
}

const uint8_t *S9xSRFGetRamWriteTrace (uint32_t *size)
{
	if (size)
		*size = srf_ram_write_trace_size;
	return srf_ram_write_trace;
}

static void srf_extend_track_visibility (void)
{
	(void) srf_draw_distance_mode;
}

static void srf_restore_track_visibility (void)
{
}
...
#include "srf_geometry_probe.h"
#include "srf_wide_capture.h"

void S9xSuperFXExec (void)
{
	uint8_t address_valid;
	uint16_t GSUStatus;
	uint64_t srf_profile_start = srf_profile_now_ns();
	srf_gsu_exec_calls++;
	/* EMULATE FX CHIP*/
	...
	fx_readRegisterSpace();
	if (srf_trace_active && srf_trace_size + 520 <= sizeof(srf_trace))
	{
		uint32_t record = srf_trace_size;
		srf_trace[record + 0] = (uint8_t) GSU.vPrgBankReg;
		srf_trace[record + 1] = (uint8_t) R15;
		srf_trace[record + 2] = (uint8_t) (R15 >> 8);
		srf_trace[record + 3] = (uint8_t) GSU.vStatusReg;
		srf_trace[record + 4] = (uint8_t) GSU.vCacheBaseReg;
		srf_trace[record + 5] = (uint8_t) (GSU.vCacheBaseReg >> 8);
		srf_trace[record + 6] = GSU.bCacheActive ? 1 : 0;
		srf_trace[record + 7] = 0;
		memcpy(&srf_trace[record + 8], GSU.pvCache, 512);
		srf_trace_size += 520;
	}
	srf_extend_track_visibility();
	...
	if (address_valid)
	{
		...
		if (fx_hw_timing)
		{
			...
			while (TF(G) && GSU.vCounter > 0)
			{
				...
				if (srf_probe_active) srf_observe_geometry();
				if (srf_wide_active) srf_observe_wide();
				FETCHPIPE;
				idx = (GSU.vStatusReg & 0x300) | vOpcode;
				(*fx_OpcodeTable[idx])();
				srf_gsu_instruction_count++;
	...
	/* Keep the expanded submission flags private to the polygon job. */
	if (!(GSU.vStatusReg & FLG_G))
		srf_restore_track_visibility();
	/* EOF EMULATE FX CHIP*/
	...
	srf_gsu_time_ns += srf_profile_now_ns() - srf_profile_start;
}
