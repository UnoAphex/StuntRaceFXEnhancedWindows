...
so produced severe frame-to-frame view flips.  The stabilized prototype uses
the authoritative followed-object position with the nearest immutable course
tangent to build an orthonormal chase camera.  Exact reconstruction of camera
pitch, shake, alternate views, and jump behavior still requires tracing the
final per-viewport camera parameters at the primary render-job boundary.
...
applies an additional presentation expansion.  The native mesh now applies a
2.0 calibration factor (override with `SRF_NATIVE_ROAD_SCALE`) so the road and
shoulders more closely match the original rasterized course.  This remains a
visual-only conversion and never changes the authoritative collision data.

The player proxy is driven by the live vehicle root and four child points.
Their horizontal principal axis supplies heading, while a least-squares plane
through the child points supplies pitch and roll.  Axis signs are stabilized
against the prior sample to prevent 180-degree PCA flips.  The resulting
screen-space low-poly car now steers, banks, and pitches with the original
vehicle state; it is still a proxy pending complete model-stream decoding.
