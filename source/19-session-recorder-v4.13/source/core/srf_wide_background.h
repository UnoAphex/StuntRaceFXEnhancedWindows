...
// Exact low-resolution world composite used only to identify which pixels in
// the finished PPU frame belong to the replaceable 3D/background layer.
static uint32_t srf_wide_world_reference[128][208];
...
void S9xSRFWideBackgroundReset() {
    ...
    memset(srf_wide_world_reference,0,sizeof(srf_wide_world_reference));
...
const uint8_t *S9xSRFWideWorldReference(uint32_t *size) {
    *size=sizeof(srf_wide_world_reference);
    return (const uint8_t*)srf_wide_world_reference;
}
static void srf_wide_background_line(unsigned line) {
    ...
    uint32_t* world_out=srf_wide_world_reference[line-32];
    memset(world_out,0,208*sizeof(*world_out));
    if (line==96)
    ...
    if(PPU.BG[0].BGSize || !(Memory.FillRAM[0x212c]&1)) return;
    const auto& world=PPU.BG[0];
    const unsigned wy=(line+world.VOffset+1)&((world.SCSize&2)?511:255);
    for(unsigned screen=24;screen<232;++screen) {
        const unsigned wx=(screen+world.HOffset)&((world.SCSize&1)?511:255);
        const unsigned tx=wx>>3, ty=wy>>3;
        const unsigned block=(tx>>5)+(ty>>5)*((world.SCSize&1)?2:1);
        const unsigned map=((world.SCBase<<1)+block*2048+((ty&31)*32+(tx&31))*2)&65535;
        const unsigned tile=Memory.VRAM[map]|(Memory.VRAM[(map+1)&65535]<<8);
        const unsigned u=(tile&0x4000)?7-(wx&7):wx&7;
        const unsigned v=(tile&0x8000)?7-(wy&7):wy&7;
        const unsigned base=(world.NameBase<<1)+(tile&1023)*64+v*2;
        unsigned index=0;
        for(unsigned bit=0;bit<8;++bit)
            index|=((Memory.VRAM[(base+(bit/2)*16+bit%2)&65535]>>(7-u))&1)<<bit;
        // Alpha records whether the 8bpp Super FX bitmap supplied this pixel.
        // RGB always contains the visible low-resolution world reference.
        world_out[screen-24]=index ? srf_wide_color(index) : (out[screen+256]&0x00ffffffu);
    }
}
