...
This branch adds an aspect-correct presentation foundation for the native-port
work. Resizable and ultrawide windows preserve a corrected 4:3 game image by
default instead of stretching it. `F11` or `Alt+Enter` toggles fullscreen;
number keys `1`, `2`, and `3` select corrected 4:3, raw-pixel, and stretch modes.
...
SRF_FULLSCREEN=1
SRF_ASPECT=4:3        # also: raw, stretch
SRF_FILTER=best       # also: nearest, linear
SRF_VSYNC=1
```

Filtering currently smooths scaling of the emulated framebuffer. It is not yet
polygon-edge antialiasing, and the side space in an ultrawide window does not yet
show additional world geometry. See [PORTING.md](PORTING.md) for the verified
baseline and the native-renderer/60-FPS plan.
