...
bool snesrecomp_save_state(const char *path) {
    ...
    free(buf);
    if (ok) printf("srf: saved state -> %s (%d bytes)\n", path, size);
    else    fprintf(stderr, "srf: save state failed (%s)\n", path);
    return ok;
}
...
bool snesrecomp_load_state(const char *path) {
    if (!s_snes || !path) return false;
    FILE *f = fopen(path, "rb");
    if (!f) { fprintf(stderr, "srf: no save state at %s\n", path); return false; }
    ...
    fclose(f);
    if (ok) printf("srf: loaded state <- %s\n", path);
    else    fprintf(stderr, "srf: load state failed (%s) — wrong ROM or corrupt?\n", path);
    return ok;
}
