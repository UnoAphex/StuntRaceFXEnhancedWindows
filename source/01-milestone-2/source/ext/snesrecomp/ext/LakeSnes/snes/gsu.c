...
static uint32_t s_trace_launch = 0;
static uint8_t s_trace_last_bank[32];
static uint16_t s_trace_last_pc[32];
static unsigned s_trace_last_pos = 0;
static unsigned s_trace_last_count = 0;
static bool s_insn_trace_started = false;

static uint16_t gsu_get_sfr(Gsu *gsu);

static uint32_t gsu_trace_hash_ram(const Gsu *gsu) {
    uint32_t hash = 2166136261u;
    if (!gsu->ram) return hash;
    for (uint32_t i = 0; i < gsu->ram_size; i++) {
        hash ^= gsu->ram[i];
        hash *= 16777619u;
    }
    return hash;
...
static void gsu_trace_close(void) {
    if (s_trace_file) fclose(s_trace_file);
    s_trace_file = NULL;
}

static bool gsu_trace_open(void) {
    if (s_trace_file) return true;
    const char *path = getenv("SRF_GSU_TRACE");
    if (!path || !*path) return false;
    if (!strcmp(path, "1")) path = "srf_gsu_trace.jsonl";
    s_trace_file = fopen(path, "wb");
    if (!s_trace_file) {
        fprintf(stderr, "GSU trace: could not open %s\n", path);
        return false;
    }
    setvbuf(s_trace_file, NULL, _IONBF, 0);
    atexit(gsu_trace_close);
    fprintf(stderr, "GSU trace: writing %s\n", path);
    return true;
}

static bool gsu_trace_begin(Gsu *gsu, GsuTraceStart *start) {
    if (!gsu_trace_open()) return false;
    Snes *snes = (Snes *)gsu->snes;
    start->launch = s_trace_launch++;
    start->frame = snes ? snes->frames : 0;
    start->cpu_bank = snes && snes->cpu ? snes->cpu->k : 0;
    start->cpu_pc = snes && snes->cpu ? snes->cpu->pc : 0;
    start->program_bank = gsu->pbr;
    start->program_pc = gsu->r[15];
    memcpy(start->r, gsu->r, sizeof(start->r));
    start->cycles = gsu->cycles;
    start->ram_hash = gsu_trace_hash_ram(gsu);
    const char *ram_dump = getenv("SRF_GSU_RAM_DUMP");
    if (ram_dump && *ram_dump && start->launch == 0 && gsu->ram) {
        FILE *dump = fopen(ram_dump, "wb");
        if (dump) {
            fwrite(gsu->ram, 1, gsu->ram_size, dump);
            fclose(dump);
        }
    }
    return true;
...
static void gsu_trace_end(Gsu *gsu, const GsuTraceStart *start) {
    if (!s_trace_file) return;
    fprintf(s_trace_file,
        "{\"launch\":%u,\"frame\":%u,\"cpu\":\"%02X:%04X\","
        "\"program\":\"%02X:%04X\",\"cycles\":%llu,\"stopped\":%s,"
        "\"pbr\":%u,\"rombr\":%u,\"rambr\":%u,\"cbr\":%u,"
        "\"scbr\":%u,\"scmr\":%u,\"colr\":%u,\"por\":%u,"
        "\"cfgr\":%u,\"clsr\":%u,\"sfr\":%u,\"height\":%d,\"bpp\":%d,"
        "\"ram_before\":\"%08X\",\"ram_after\":\"%08X\",\"r_before\":[",
        start->launch, start->frame, start->cpu_bank, start->cpu_pc,
        start->program_bank, start->program_pc,
        (unsigned long long)(gsu->cycles - start->cycles),
        gsu->flag_go ? "false" : "true", gsu->pbr, gsu->rombr,
        gsu->rambr, gsu->cbr, gsu->scbr, gsu->scmr, gsu->colr, gsu->por,
        gsu->cfgr, gsu->clsr, gsu_get_sfr(gsu),
        gsu_screen_height(gsu), gsu_screen_bpp(gsu), start->ram_hash,
        gsu_trace_hash_ram(gsu));
    for (int i = 0; i < 16; i++) fprintf(s_trace_file, "%s%u", i ? "," : "", start->r[i]);
    fprintf(s_trace_file, "],\"r_after\":[");
    for (int i = 0; i < 16; i++) fprintf(s_trace_file, "%s%u", i ? "," : "", gsu->r[i]);
    fprintf(s_trace_file, "],\"program_after\":\"%02X:%04X\",\"last_pc\":[",
            gsu->pbr, gsu->r[15]);
    unsigned first = s_trace_last_count < 32 ? 0 : s_trace_last_pos;
    for (unsigned i = 0; i < s_trace_last_count; i++) {
        unsigned slot = (first + i) & 31;
        fprintf(s_trace_file, "%s\"%02X:%04X\"", i ? "," : "",
                s_trace_last_bank[slot], s_trace_last_pc[slot]);
    }
    fprintf(s_trace_file, "]}\n");
}
...
/* The queued byte is the architectural delay slot. This interpreter fetches
 * at PC+1, so retain PC one byte before a taken destination; the next pipe
 * step fetches the destination while executing that queued delay slot. */
static void gsu_take_branch(Gsu *gsu, int8_t offset) {
    PC = (uint16_t)(PC + offset - 1);
}

/* BRA — unconditional branch (signed 8-bit offset) */
static void gsu_op_bra(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_bge(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    if (gsu->flag_s == gsu->flag_ov) gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_bne(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    if (!gsu->flag_z) gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_bpl(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    if (!gsu->flag_s) gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_bcc(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    if (!gsu->flag_cy) gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_bvc(Gsu *gsu) {
    int8_t offset = (int8_t)gsu_pipe(gsu);
    if (!gsu->flag_ov) gsu_take_branch(gsu, offset);
    gsu_step(gsu, 1);
}
...
static void gsu_op_iwt(Gsu *gsu, int n) {
    if (gsu->flag_alt1) {
        /* LM Rn, (xx); ALT3 also selects LM because ALT1 has priority. */
...
static void gsu_op_ibt(Gsu *gsu, int n) {
    if (gsu->flag_alt1) {
        /* LMS Rn, (xx); ALT3 also selects LMS. */
        uint8_t imm = gsu_pipe(gsu);
        uint16_t addr = (uint16_t)imm << 1;
        gsu_ram_buffer_load(gsu, addr);
        uint8_t a = gsu->ramdr;
...
static void gsu_op_jmp(Gsu *gsu, int n) {
    if (gsu->flag_alt1) {
        ...
        gsu->cbr = gsu->r[n] & 0xFFF0;
        PC = (uint16_t)(gsu->r[n] - 1);
        gsu_cache_flush(gsu);
    } else {
        PC = (uint16_t)(gsu->r[n] - 1);
    }
...
static void gsu_execute(Gsu *gsu, uint8_t op) {
    ...
    /* $10-$1F: TO Rn, or MOVE Rn,SREG while WITH's B flag is set. */
    case 0x10: case 0x11: case 0x12: case 0x13:
    case 0x14: case 0x15: case 0x16: case 0x17:
    case 0x18: case 0x19: case 0x1A: case 0x1B:
    case 0x1C: case 0x1D: case 0x1E: case 0x1F:
        gsu_op_to(gsu, op & 0x0F);
        return; /* TO is a prefix; MOVE resets its own prefix state. */

    /* $20-$2F: WITH Rn (select source+destination and set B). */
    ...
    case 0x4C:
        if (gsu->flag_alt1) gsu_op_rpix(gsu);
        else gsu_op_plot(gsu);
        break;
    ...
    /* $B0-$BF: FROM Rn, or MOVES DREG,Rn while WITH's B flag is set. */
    case 0xB0: case 0xB1: case 0xB2: case 0xB3:
    case 0xB4: case 0xB5: case 0xB6: case 0xB7:
    case 0xB8: case 0xB9: case 0xBA: case 0xBB:
    case 0xBC: case 0xBD: case 0xBE: case 0xBF:
        gsu_op_from(gsu, op & 0x0F);
        return; /* FROM is a prefix; MOVES resets its own prefix state. */

    /* $C0-$CF: HIB / OR / XOR (register or immediate). */
    ...
    case 0xDC: case 0xDD: case 0xDE:
        ...
        break;
    case 0xDF:
        /* GETC; ALT2 RAMB; ALT3 ROMB. ALT1 still behaves as GETC. */
        if (gsu->flag_alt2 && gsu->flag_alt1) {
            gsu_op_romb(gsu);
        } else if (gsu->flag_alt2) {
            gsu_op_ramb(gsu);
        } else {
    ...
    /* $E0-$EE: DEC Rn. $EF: GETB and its ALT variants. */
    ...
    case 0xEC: case 0xED: case 0xEE:
        ...
        break;
    case 0xEF:
        if (gsu->flag_alt1 && gsu->flag_alt2) gsu_op_getbs(gsu);
        else if (gsu->flag_alt1) gsu_op_getbh(gsu);
        else if (gsu->flag_alt2) gsu_op_getbl(gsu);
        else gsu_op_getb(gsu);
        break;
    ...
    /* Every opcode reaching this epilogue is a completed instruction. The
       TO/WITH/FROM and ALT prefix opcodes return above. */
    gsu_reset_prefix(gsu);
}
...
void gsu_write(Gsu *gsu, uint16_t addr, uint8_t val) {
    ...
    if (addr < 0x20) {
        ...
        if (addr & 1) {
            ...
            if (reg == 15) {
                GsuTraceStart trace;
                bool tracing = gsu_trace_begin(gsu, &trace);
                gsu->flag_go = true;
                gsu->pipeline_valid = false;
                gsu_run(gsu);
                if (tracing) gsu_trace_end(gsu, &trace);
            }
        } else {
...
void gsu_run(Gsu *gsu) {
    ...
    const char *limit_env = getenv("SRF_GSU_CYCLE_LIMIT");
    if (limit_env && *limit_env) limit = strtoull(limit_env, NULL, 10);
    bool record_last_pc = getenv("SRF_GSU_TRACE") != NULL;
    FILE *insn_trace = NULL;
    const char *insn_path = getenv("SRF_GSU_INSN_TRACE");
    if (insn_path && *insn_path && !s_insn_trace_started) {
        insn_trace = fopen(insn_path, "wb");
        s_insn_trace_started = insn_trace != NULL;
    }
    unsigned insn_count = 0;
    s_trace_last_pos = 0;
    s_trace_last_count = 0;

    gsu_peekpipe(gsu);

    while (gsu->flag_go && (gsu->cycles - start) < limit) {
        if (record_last_pc) {
            s_trace_last_bank[s_trace_last_pos] = gsu->pbr;
            s_trace_last_pc[s_trace_last_pos] = gsu->r[15];
            s_trace_last_pos = (s_trace_last_pos + 1) & 31;
            if (s_trace_last_count < 32) s_trace_last_count++;
        }
        uint16_t trace_pc = gsu->r[15];
        uint8_t op = gsu_pipe(gsu);
        if (insn_trace && insn_count++ < 20000) {
            fprintf(insn_trace,
                    "%u %02X:%04X op=%02X sfr=%04X r0=%04X r1=%04X r2=%04X r3=%04X r12=%04X r13=%04X r14=%04X\n",
                    insn_count, gsu->pbr, trace_pc, op, gsu_get_sfr(gsu),
                    gsu->r[0], gsu->r[1], gsu->r[2], gsu->r[3],
                    gsu->r[12], gsu->r[13], gsu->r[14]);
        }
        gsu_execute(gsu, op);
    }
    if (insn_trace) fclose(insn_trace);
}
