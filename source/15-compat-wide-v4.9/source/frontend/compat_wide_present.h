...
static float wide_cross(SDL_FPoint a, SDL_FPoint b, SDL_FPoint c) {
...
static void draw_compat_wide_sides(const SDL_Rect& center) {
    ...
    for (size_t offset=0; offset<compat_wide_display_bytes.size(); offset+=sizeof(CompatWideCameraFace)) {
        ...
        SDL_FPoint a[64], b[64];
        int clipped = 0;
        for (int i = 0; i < count; ++i) {
            const float* p = &face.xyz[i*3];
            const float* q = &face.xyz[((i+1)%count)*3];
            if(p[2] >= 1) a[clipped++] = {face.center_x+128*p[0]/p[2], face.center_y+128*p[1]/p[2]};
            if((p[2] >= 1) != (q[2] >= 1)) {
        ...
        for (int edge = 0; edge < 4 && count; ++edge) {
            int output = 0;
            for (int i = 0; i < count; ++i) {
                const SDL_FPoint p = a[i], q = a[(i+1)%count];
                ...
                if (pin && output < 64) b[output++] = p;
                if (pin != qin && output < 64) {
                    const float t = (limit-pv)/(qv-pv);
                    b[output++] = {p.x+t*(q.x-p.x), p.y+t*(q.y-p.y)};
                }
            }
            count = output;
        ...
        const uint16_t rgb = compat_wide_palette[face.color&255];
        const SDL_Color color{static_cast<Uint8>((rgb&31)*255/31),
            static_cast<Uint8>(((rgb>>5)&31)*255/31), static_cast<Uint8>(((rgb>>10)&31)*255/31),255};
        int indices[64];
        ...
        while(count >= 3) {
            ...
            for(int i=0;i<count;++i) {
                ...
                for(int k : {ia,ib,ic}) compat_wide_triangles.push_back({
                    {origin_x+a[k].x*sx,origin_y+a[k].y*sy},color,{0,0}});
    ...
    const int left = static_cast<int>(origin_x), right = static_cast<int>(origin_x+208*sx);
    ...
    for(const SDL_Rect& side : sides) {
        ...
        if(!compat_wide_triangles.empty()) SDL_RenderGeometry(renderer,nullptr,
            compat_wide_triangles.data(),static_cast<int>(compat_wide_triangles.size()),nullptr,0);
    }
