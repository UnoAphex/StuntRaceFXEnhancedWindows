# Reconstruction session recording (retained in v4.15)
...
```text
python source/21-road-material-fix-v4.15/source/tools/recording/inspect_recording.py recordings/<session>/session.srfxrec --verify
```

Reconstruct and extract the complete accumulated state at a chosen frame:

```text
python source/21-road-material-fix-v4.15/source/tools/recording/inspect_recording.py recordings/<session>/session.srfxrec --verify --frame 2500 --output <folder>
```
