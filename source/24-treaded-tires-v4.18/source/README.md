# Stunt Race FX Treaded Tire Test v4.18 — source bundle

Version 24 adds an opt-in HD compatibility-renderer experiment that overlays a
generated treaded tire asset on the measured source-space footprints of the
player's two large rear wheels. The original wheels are Super FX vehicle
geometry rather than OBJ sprites, so this is a presentation-only substitution;
gameplay state and native-renderer behavior are unchanged. See
TREADED_TIRE_TEST.md.

## Inherited v4.17 timing baseline

Version 23 makes the corrected timing architecture explicit across every
compatibility-renderer preset. Core video submissions are retained, but the
compatibility renderer now presents exactly once after each simulation tick,
eliminating the duplicate presentation discovered on startup frame 2. Every
compatibility launcher uses the same native-rate deadline accumulator, limiter,
non-runahead baseline, and diagnostics. See COMPATIBILITY_PRESET_TIMING.md.

## Inherited v4.16 material correction
