...
The matrix scratch block at GSU `$00E4` is reused by palette and secondary
render jobs and cannot be sampled blindly as a persistent final camera. Doing
so produced severe frame-to-frame view flips. The v4.6 prototype uses the
authoritative followed-object position and ordered live wheel pose to build its
chase camera, with the nearest immutable course tangent as fallback. Exact
reconstruction of camera pitch, shake, alternate views, and jump behavior still
requires tracing final per-viewport parameters at the primary render boundary.
...
- a stable low-poly player presentation and live opponent diagnostic proxies;
- wireframe, segment IDs, and cache/culling counters.

Hybrid comparison established that the final word in each course node behaves
as the collision/search half-width, while the original visible road builder
applies an additional presentation expansion. XLR8 source inspection then
verified `track_scale=5` and authored road-piece half widths commonly spanning
960–2560 world units. The native mesh therefore applies a 4.0 calibration
factor by default (override with `SRF_NATIVE_ROAD_SCALE`). This remains a
visual-only conversion and never changes authoritative collision data.

The native vehicle proxy is driven by the live vehicle root and four child
wheel points. XLR8's ordered wheel definitions establish axle pairing, allowing
front/rear axle centers to provide directed heading and pitch while side
centers provide roll and track width. The player proxy is now world-space and
the chase camera follows its live heading. PCA remains only as a fallback for
incomplete child records. The geometry remains a proxy pending retail
model-stream decoding.

Native presentation is gated by the race-only NMI cycle and a continuous race
warm-up interval.  Title screens, selection menus, loading masks, grid views,
and the countdown therefore stay on the complete compatibility framebuffer.
The cached course may remain resident in memory, but it is not presented merely
because a stale course pointer still exists.
...
- `[` / `]`: vertical FOV
- `;` / `'`: narrow/widen the native road calibration
- `-` / `=`: native far distance
