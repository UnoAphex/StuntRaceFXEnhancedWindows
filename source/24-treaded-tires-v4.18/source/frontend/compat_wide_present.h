...
// the recurring road faces. Keep this deliberately narrow:
...
// or gravel. Green polygon slots and the BG2 layer are intentionally excluded:
// the recording proves neither is a reliable terrain-ownership signal. The RGB
// check makes the rule self-disabling when another scene repurposes a slot.
...
// The original player wheels are Super FX-rendered vehicle geometry, not SNES
// OBJ sprites. This optional HD presentation pass lays the generated tread art
// over their measured source-space footprints. Using source coordinates keeps
// the replacement proportional at every output resolution without affecting
// the authoritative vehicle, collision, input, or simulation state.
static void draw_treaded_player_tires(const SDL_Rect& center) {
    if (!treaded_tires || !treaded_tire_texture || !race_scene_active ||
        !compat_hd_center || render_path != RENDER_COMPATIBILITY ||
        !source_width || !source_height) return;
    const float sx = static_cast<float>(center.w) / source_width;
    const float sy = static_cast<float>(center.h) / source_height;
    const SDL_FRect left{
        center.x + 87.0f * sx, center.y + 102.0f * sy, 28.0f * sx, 41.0f * sy};
    const SDL_FRect right{
        center.x + 138.0f * sx, center.y + 102.0f * sy, 28.0f * sx, 41.0f * sy};
    SDL_RenderCopyF(renderer, treaded_tire_texture, nullptr, &left);
    SDL_RenderCopyExF(renderer, treaded_tire_texture, nullptr, &right,
                     0.0, nullptr, SDL_FLIP_HORIZONTAL);
    active_profile.render_copies += 2;
}
