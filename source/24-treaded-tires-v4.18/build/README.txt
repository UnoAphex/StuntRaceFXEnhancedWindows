STUNT RACE FX - TREADED TIRE TEST v4.18
========================================

v4.18 TREADED TIRE TEST
-----------------------
This build adds an optional high-resolution tread treatment for the player's
two large rear wheels in the HD compatibility preview. Launch
"Play Treaded Tire Test 1920x1080.cmd" and supply your own USA Rev 1 ROM.

Press Ctrl+Y during a race to switch between the treaded replacements and the
original Super FX wheels. The replacement follows the measured source-space
wheel footprint and therefore scales proportionally with the output window.
It changes presentation only: handling, collision, vehicle state, timing,
interpolation, and the native renderer are unchanged.

The rear wheels are not ordinary SNES sprite assets. They are produced by the
Super FX vehicle geometry path, so this first experiment is a matched overlay
for visual evaluation rather than a modification of the original model data.

INHERITED UNIFIED COMPATIBILITY TIMING v4.17
--------------------------------------------

v4.17 COMPATIBILITY PRESET TIMING
---------------------------------
Every compatibility preset now presents through the same timing boundary.
The frontend accepts every source video update from the emulation core, keeps
the newest one, and presents exactly once per simulation tick. This removes a
duplicate presentation that occurred during startup without changing input,
gameplay, physics, sprite timing, or simulation speed.

All player-facing compatibility launchers explicitly use the native 60.0988 Hz
cadence, the same high-resolution deadline accumulator, no runahead, and no
second SDL VSync limiter. Resolution, widescreen, HD center, materials, edge
smoothing, windowed/fullscreen, and interpolation settings no longer depend on
implicit inherited timing options.

The profiler records source video callbacks separately from actual presented
frames and reports missing/duplicate presentations. All 15 compatibility
configurations, including explicit VSync and fullscreen cases, passed
deterministic timing and first-race coverage tests.

INHERITED v4.16 CORRECTION
--------------------------

The automatic green BG2 grass treatment has been removed. Runtime testing
showed that this layer can also appear beneath or participate in the near road,
so green pixels alone cannot safely distinguish terrain from asphalt. Enhanced
materials now affect only the two verified asphalt road-face signatures.

Grass will remain in its original flat presentation until course/collision data
can provide a reliable road-versus-terrain mask. Press T to compare materials.

INHERITED v4.15 CORRECTION
--------------------------

The grass texture is now confined to the BG2 landscape below the horizon.
Green polygon faces are no longer assumed to be terrain because the game also
reuses that color family for road/shoulder geometry. Verified road faces retain
the asphalt treatment. Press T to compare enhanced and original materials.

INHERITED v4.14 ENHANCED FIRST-TRACK QUICK START
------------------------------------------------

1. Put your legally obtained USA Rev 1 ROM beside StuntRaceFX.exe.
2. Name it exactly: Stunt Race FX (USA) (Rev 1).sfc
3. Launch "Play First Track Enhanced 3440x1440.cmd".

This build uses the complete recorded first-course session to distinguish road
faces from vehicle/scenery colors and to identify the large turf layer carried
by BG2. The output-resolution world pass now applies restrained asphalt detail
to verified road faces and grass detail to the broad green landscape. Original
colors, texture-mapped objects, cars, signs, sky, HUD, and sprites are retained.

Press T to toggle the material treatment for comparison. The two True Wide HD
Preview launchers also start with this treatment enabled. The normal 4:3 path,
gameplay, timing, interpolation, controls, sprite animation, and native renderer
are unchanged. No recording or ROM-derived export is included in this package.

The v4.13 recorder remains available through
"Record Reconstruction Session 3440x1440.cmd".

INHERITED v4.13 DATA-RECORDING QUICK START
------------------------------------------

1. Put your legally obtained USA Rev 1 ROM beside StuntRaceFX.exe.
2. Name it exactly: Stunt Race FX (USA) (Rev 1).sfc
3. Launch "Record Reconstruction Session 3440x1440.cmd".
4. Play normally and close the program cleanly when finished.

Inside the Git checkout, every run receives its own timestamped folder under:

  C:\StuntRaceFXWE\recordings\session-YYYYMMDD-HHMMSS-pPROCESSID\

The folder contains manifest.json, frames.csv, and session.srfxrec. When the
release is extracted outside the checkout, recordings are kept beside the
executable instead. Ctrl+Shift+R stops or starts reconstruction recording.

The session stores the complete reconstruction-facing stream: polygon and
camera packets, display geometry, palette, PPU and DMA state, VRAM, background,
world reference, identified sprites, original framebuffer, separated HD overlay,
Super FX RAM/registers/traces, WRAM, controller input, timing, and periodic exact
emulator checkpoints. The ROM itself is never copied into the recording.

Data is losslessly deduplicated and compressed. The deterministic 2,500-frame
first-track test produced approximately 93 MB instead of 1.09 GB of raw data.
Long sessions can still require gigabytes, so keep adequate free disk space.

Recordings are ROM-derived personal research data. Keep them local; do not add
them to Git or distribute them. The repository protects its root recordings
folder with .gitignore. Normal launchers do NOT record.

See ../source/RECONSTRUCTION_RECORDING.md for the channel format and inspection
tool. v4.12 HD-wide rendering, timing, interpolation, and 2D composition remain
unchanged. The experimental native renderer remains unchanged.

INHERITED v4.12 HD-WIDE QUICK START
-----------------------------------

1. Put your legally obtained USA Rev 1 ROM beside StuntRaceFX.exe.
2. Name it exactly: Stunt Race FX (USA) (Rev 1).sfc
3. Launch "Play True Wide HD Preview 1920x1080.cmd" or
   "Play True Wide HD Preview 3440x1440.cmd".

The HD preview renders captured original 3D polygons on the GPU at the selected
PC output resolution across both the center and genuinely expanded sides. It
does not stretch the original 4:3 world. The original HUD, text, wheel animation,
and other non-polygon graphics are kept in a separate proportional overlay.

CONTROLS
--------
- Ctrl+H: toggle the full HD world preview.
- Ctrl+W: toggle expanded side viewports.
- Alt+Enter or F11: toggle fullscreen/windowed.
- F12: toggle FPS and performance details.

The standard compatibility launchers remain the conservative default. At 4:3,
the wide extension is bypassed and the original compatibility result is
unchanged. The 60 FPS presentation, responsive input handling, and independent
2D animation timing are inherited unchanged.

CURRENT PREVIEW LIMITS
----------------------
- Objects fully rejected by the original Super FX viewport before their faces
  are submitted cannot yet be recovered by this read-only compatibility path.
- Small unsupported Super FX bitmap primitives are restored from the original
  image and therefore retain their original pixel character.
- The distant tile background retains original geometry/texel density; v4.14
  adds optional restrained turf detail without replacing its original art.
- This is not the experimental native renderer; that code remains unchanged.

See ../source/COMPATIBILITY_HD_WIDE.md in the repository for implementation and
measured validation details.

INHERITED v4.11 OBJECT RESTORATION
----------------------------------
...
V4.11 LIMITS (SUPERSEDED FOR THE CENTER BY v4.12 HD MODE)
---------------------------------------------------------
...
In v4.11 the center remained the original-resolution compatibility image. The
new v4.12 HD mode supplies the GPU center reconstruction and separate overlay;
these notes are retained only as historical context.
...
In v4.11 the center was still the original-resolution compatibility image. The
v4.12 HD launcher replaces that limitation with output-resolution center
geometry and reliable 2D overlay separation.
