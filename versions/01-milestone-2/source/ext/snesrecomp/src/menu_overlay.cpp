...
#define SMK_CONFIG_PATH "srf_config.ini"
#define SMK_REPO_URL "https://github.com/sp00nznet/stuntrace"
...
static void config_save(void) {
    ...
    if (!f) return;
    fprintf(f, "# Stunt Race FX - Enhanced Recompilation settings\n");
    fprintf(f, "scale=%d\n",     g.scale);
    fprintf(f, "vsync=%d\n",     g.vsync);
    fprintf(f, "filter=%d\n",    g.filter);
...
extern "C" void menu_overlay_init(struct SDL_Window *window, struct SDL_Renderer *renderer) {
    set_default_binds(g);
    if (getenv("SRF_HEADLESS") || getenv("SRF_SCRIPT") || getenv("SRF_MAX_FRAMES") ||
        getenv("SMK_HEADLESS") || getenv("SMK_SCRIPT") || getenv("SMK_MAX_FRAMES")) {
        g.enabled = false;
        return;
    }
