...
static void load_input_script(void) {
    ...
    if (!file) {
        fprintf(stderr, "Input script: could not open %s\n", path);
        return;
    }

    char line[160];
    while (s_script_count < (int)(sizeof(s_script) / sizeof(s_script[0])) &&
           fgets(line, sizeof(line), file)) {
        if (line[0] == '#' || line[0] == '\n') continue;
        unsigned first = 0, last = 0, mask = 0;
        if (sscanf(line, "%u %u %x", &first, &last, &mask) != 3 || last < first)
            continue;
        s_script[s_script_count++] = (ScriptedInput){ first, last, (uint16_t)mask };
    }
    fclose(file);
    fprintf(stderr, "Input script: loaded %d events from %s\n", s_script_count, path);
}

static uint16_t scripted_mask(uint32_t frame) {
    uint16_t mask = 0;
    for (int i = 0; i < s_script_count; i++) {
        if (frame >= s_script[i].first_frame && frame <= s_script[i].last_frame)
            mask |= s_script[i].mask;
    }
    return mask;
}
...
void recomp_input_update(void) {
    ...
    static const uint16_t button_masks[12] = {
        SNES_BTN_B, SNES_BTN_Y, SNES_BTN_SELECT, SNES_BTN_START,
        SNES_BTN_UP, SNES_BTN_DOWN, SNES_BTN_LEFT, SNES_BTN_RIGHT,
        SNES_BTN_A, SNES_BTN_X, SNES_BTN_L, SNES_BTN_R
    };
    load_input_script();
    uint16_t script_p1 = scripted_mask(s_input_frame++);
    ...
    if (s_device_type[0] == SNES_INPUT_MOUSE) {
        ...
        for (int btn = 0; btn < 12; btn++) {
            bool physical = block ? false : button_pressed(1, btn, keys);
            snes_setButtonState(snes, 1, btn, physical || (script_p1 & button_masks[btn]));
        }
    }
